...
module.exports = wrap(async function (req, res) {
	...
	const playerRes = await makeRequest(
		res,
		`https://api.hypixel.net/v2/player?key=${req.query.key?.toString()}&uuid=${uuid}`
	);
	const player = parseHypixel(playerRes, uuid, res);

	const profileRes = await makeRequest(
		res,
		`https://api.hypixel.net/v2/skyblock/profiles?key=${req.query.key?.toString()}&uuid=${uuid}`
	);
	const profile = await parseProfiles(player, profileRes, uuid, res, req.query.key?.toString());
