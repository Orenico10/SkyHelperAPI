...
module.exports = (player, profile) => {
	...
	for (const perk in Object.keys(mining_stats?.nodes || {}) || []) {
		if (!Object.keys(mining_stats.nodes)[perk].startsWith('toggle_')) {
			...
			if (!currentPerk) continue;
			player_perks.push({
				name: currentPerk?.name,
				id: currentPerk?.id,
				level: Object.values(mining_stats.nodes)[perk],
	...
	if (mining_stats?.nodes?.quick_forge) {
		for (const item of Object.keys(forgeItemTimes)) {
			...
			forgeItemTimes[item].duration =
				forgeItemTimes[item].duration - forgeItemTimes[item].duration * lessForgingTime;
		}
	}
	...
	return {
		...
		mithril_powder: {
			current: mining_stats?.powder_mithril_total || 0,
			total: (mining_stats?.powder_mithril_total || 0) + (mining_stats?.powder_spent_mithril || 0),
		},
		gemstone_powder: {
			current: mining_stats?.powder_gemstone_total || 0,
			total: (mining_stats?.powder_gemstone_total || 0) + (mining_stats?.powder_spent_gemstone || 0),
		},
		hotM_tree: {
			tokens: {
