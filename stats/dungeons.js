		...
		for (const floor in master_available_floors) {
			if (floor != 0) {
				master_mode_floors[`floor_${floor}`] = {
					...
					completions: master_catacombs?.tier_completions
						? master_catacombs?.tier_completions[floor] || 0
						: 0,
					best_score: {
					...
					fastest_s_plus: master_catacombs?.fastest_time_s_plus
						? master_catacombs?.fastest_time_s_plus[floor] || 0
						: 0,
